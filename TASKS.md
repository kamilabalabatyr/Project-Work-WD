...
- [+] Написать секцию **API Documentation** в `README.md` _(сделал Саша)_

### Фаза 6: Финализация (День 6–7)
- [ ] Протестировать интеграцию с фронтендом
- [+] Обработать edge cases: двойное бронирование на те же даты, чужое удаление _(коллизия дат + IsOwnerOrReadOnly — Саша)_
...
- [+] **`AuthInterceptor`** (`interceptors/auth.interceptor.ts`)
  ...
  ```
- [+] Зарегистрировать в `app.config.ts` через `provideHttpClient(withInterceptors(...))`
...
|---|---|---|
| 4+ Models | Денис | ✅ (User, Property, Booking, Review) |
| 2+ ForeignKey | Денис | ✅ (4 FK: owner, property, guest, booking) |
| 2 Serializer + 2 ModelSerializer | Саша + Денис | ✅ (Register, Login + Property, Booking, Review) |
| 2 FBV + 2 CBV | Саша | ✅ (register, login, logout + PropertyList, PropertyDetail, BookingList, BookingDetail) |
| Token Auth (login/logout) | Саша | ✅ |
| Full CRUD (Property) | Саша | ✅ |
| request.user binding | Саша | ✅ |
| CORS | Саша | ✅ |
| Postman collection | Денис | ✅ |
| Angular routing 3+ pages | Камила | ✅ (5 страниц: home, properties, property-detail, login, my-bookings) |
| Services + Interfaces | Камила | ✅ (3 сервиса + 3 интерфейса) |
| 4+ API events | Камила | ✅ (5+: getAll, getById, create property, create booking, delete) |
| 4+ ngModel inputs | Камила | ✅ (10+ inputs: фильтры, формы, бронирование, логин) |
| JWT Interceptor | Камила | ✅ (Token interceptor + 401 auto-logout) |
| @for / @if | Камила | ✅ |
| Error handling | Камила | ✅ |
| README.md | Саша | ✅ |
| Postman в репо | Денис | ✅ (postman_collection.json) |
| 4 слайда PDF | Денис | ⬜ |
